...
async function loadBlogData() {
  try {
    const blogJsonPath = import_path.default.join(process.cwd(), "blog.json");
    const data = await import_promises.default.readFile(blogJsonPath, "utf-8");
...
app.get("*", (req, res) => {
  ...
  if (!import_path.default.extname(requestPath)) {
...
\u{1F680} Development server running at http://localhost:${PORT}`);
  ...
  const watcher = import_chokidar.default.watch(["blog.json", "src/**/*.ts", "templates/**/*", "assets/**/*"], {
    persistent: true,
    ignoreInitial: true
  });
