...
The bot is a bit finicky to get working. It currently only works on Windows (to my chagrin, as a lifelong user of Linux).

I encountered varying limitations at every step of getting Omega to work e.g. audio drivers, OS restrictions for different tools in the chain. The code isn't as good I would like it to be.
...
The Node.js client listens to audio output from Clubdeck, transcribes the spoken words and sends a transcription and prompt **(requires clarification)** to GPT-3. Upon receiving a response from GPT-3, the client converts this text to synthesized speech and then delivers this as an audio stream to Clubdeck's virtual microphone.
...
```
The following is a conversation with ${botName}. ${botName} is an AI bot created by Ajax. ${botName} enjoys having philosophical conversations. ${botName} is sitting in a room with other people, having a discussion. ${botName} is extremely intelligent, funny, sardonic and humble. He is talking to other people in the room.
``` 

**(This sentence seems largely redundant. Most of this information was conveyed in the 4th sentence, except ${botName}'s gender pronoun which is new information, but it could be included elsewhere.)**
...
As people talk and as the persona replies, everything is recorded in a transcript file so the bot retains an idea of everything that has been previously said. We're only using six to eight **(Changed 4-8 to 6-8)** previous messages from the log:
...
```
**(Add a couple of more messages above?**
...
Omega sometimes hangs out with people on Clubhouse and is more popular there than me. You can find Omega on Clubhouse @omegadone.
